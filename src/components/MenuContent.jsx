...
function MenuContent() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      el.scrollBy({ left: 300 * directionRef.current, behavior: "smooth" });
    }, 2500);
    ...
    <div className="w-[80%] mx-auto min-w-0 flex flex-col gap-10 ">
            ...
            <div className="w-full lg:w-[60%] flex justify-between ">
                {types.map((type) => (
                    <button 
                        ...
                        className={`border rounded-3xl border-[#313F5E] text-[#313F5E] lg:px-6 px-2 lg:py-2 text-[10px] md:text-[14px] lg:text-[16px] transition-all duration-300
      ...
      >
        {filteredDishes.map((dish, i) => (
          <div
            key={dish.name + i}
            className="shrink-0 w-[320px] border border-[#AFBBD5] shadow h-60 rounded-4xl  p-2 pl-14 relative flex flex-col gap-2"
          >
